...
public class StayMoveLogic extends Thread {
...
//	private static final int MAX_TRACE_TIME_GAP = 30 * 60 * 1000; //max time without receiving traces, for which a previous stay event is considered to be connected to the traces received after the gap
	...
	public StayMoveLogic(EventDAL eventDal, TraceDAL traceDal, UserDAL userDal, EventGenerationInfo genInfo) {
	...
	public void handleTrace(PositionTrace currentTrace) {
		...
		Event currentEvent = genInfo.getCurrentPositionEvent();
		double radius = currentEvent instanceof MoveEvent ? SMALL_RADIUS : BIG_RADIUS;
		RecentTraces recent = new RecentTraces(user, currentTrace, currentEvent, TIME_WINDOW, radius);
		...
		if(recent.isLongEnough){
			...
			if (currentEvent == null) {
				...
				if (recent.isMoving) {
					...
					genInfo.setCurrentPositionEvent(newEvent);
...
//		userDal.makePersistent(genInfo);
	...
	private class RecentTraces{
		...
		RecentTraces(User user, PositionTrace currentTrace, Event currentEvent, int timeWindow, double radius){
		...
		private void load(PositionTrace currentTrace){
